...
def get_attack_stats(data_folder, target, key_file, num_samples, path_to_model=None):
...
@click.option('--model-file', default=None, help='Model checkpoint file')
...
def main(target, data_folder, mode, key_file, num_samples, model_file, traces_file, save_plots):
    if mode == 'attack':
        ...
        stats = get_attack_stats(data_folder, target, key_file, num_samples, path_to_model=model_file)
        print("Key ranks in attack trials (log2):")
        print(stats)
        print("Median key rank (log2): {}".format(np.median(stats)))
    elif mode == 'train':
        if model_file is None:
            model_file = f'./models/model_{target}.h5'
        if target not in list(net_params.keys()):
            raise ValueError('Target must be one of: {}'.format(list(net_params.keys())))
        net_parameters = net_params[target]
